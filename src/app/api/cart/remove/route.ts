...
import { removeItemSchema } from '@/validation/cart';
...
export const POST = errorHandler(async (req: Request) => {
  ...
  const { userId, sessionToken, productId } = validate(removeItemSchema, raw);
  const cart = await CartService.getOrCreate({ userId, sessionToken });
  const updated = await CartService.removeItem(cart.id, productId);
  // ...existing code...
